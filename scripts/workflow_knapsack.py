...
block_0 = InstantiateModel(
    model_class=Generator,
    name='GENERATOR',
    position=(-426.27884474719355, -197.67219288999567))
block_1 = ModelMethod(
    method_type=MethodType(Generator, 'generate'),
    name='_generate_',
    position=(-124.05791819073008, -185.06367196641366))
block_2 = InstantiateModel(
    model_class=Dataset,
    name='DATASET',
    position=(162.1159937393057, -96.39613408640047))
block_3 = ModelAttribute(
    attribute_name='dessia_objects',
    name='extract',
    position=(452.9770685403816, 1.7984784075740556))
block_4 = MultiPlot(
    selector_name="MultiPlot",
    attributes=['mass', 'price', 'golds', 'silvers', 'bronzes'],
    name='Multiplot',
    position=(781.280317117172, -141.01673915293895))
block_5 = Markdown(
    selector=MarkdownType(class_=Dataset, name='Markdown'),
    name='Markdown')
block_6 = Unpacker(
    indices=[0],
    name='extract_solution',
    position=(804.5481046770818, 112.7202979561202))
block_7 = PlotData(
    selector=PlotDataType(class_=KnapsackPackage, name='PlotData'),
    name='plot_2d')
block_8 = CadView(
    selector=CadViewType(class_=KnapsackPackage, name='CadView'),
    name='CAD')
...
variable_0 = Variable(
    name='Result Name',
    position=(-135.41769504700423, 146.07556919422217),
    type_=str)
...
pipes = [pipe_0, pipe_1, pipe_2, pipe_3, pipe_4,
         pipe_5, pipe_6, pipe_7, pipe_8]

workflow = Workflow(
    blocks=blocks,
    pipes=pipes,
    output=block_3.outputs[0],
    name='WORKFLOW KNAPSACK PROBLEM_Display')
